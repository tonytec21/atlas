...
$origemSaldoInicial = fetchData(
    'SELECT data_caixa, data_transporte, data_caixa_uso, valor_transportado, funcionario 
    FROM transporte_saldo_caixa 
    WHERE DATE(data_caixa_uso) = :data AND status = "usado"', 
    $params
);
...
$devolucoes = fetchData(
    'SELECT os.id as ordem_de_servico_id, os.cliente, do.forma_devolucao, do.total_devolucao, do.funcionario
...
$saidas = fetchData(
    'SELECT titulo, valor_saida, forma_de_saida, data, funcionario
...
$saldoInicial = fetchData(
    'SELECT funcionario, saldo_inicial
    FROM caixa 
    WHERE DATE(data_caixa) = :data', 
    $params
);
...
$totalDevolucoesEspecie = array_sum(array_map(
...
$totalEmCaixa = $totalSaldoInicial 
...
$pdf = new PDF();
$pdf->SetTitle('Fechamento de Caixa Unificado');
$pdf->AddPage();
$pdf->SetFont('helvetica', 'B', 14);
$pdf->Cell(0, 10, 'Relatório de Fechamento do Caixa Unificado - '.date('d/m/Y', strtotime($data)), 0, 1, 'C');
$pdf->Ln(2);
...
$cards = [
    ...
    'Total Recebido' => $totalDePagamentos,
...
renderTable($pdf, 'Atos Liquidados',
    ['O.S', 'CLIENTE', 'ATO', 'DESCRIÇÃO', 'QTD', 'TOTAL (R$)', 'FUNCIONÁRIO'],
    array_map(fn($a) => [
        $a['ordem_servico_id'],
        $a['cliente'],
        $a['ato'],
...
renderTable($pdf, 'Saídas e Despesas',
    ['TÍTULO', 'VALOR (R$)', 'FORMA DE PAGAMENTO', 'DATA CAIXA', 'FUNCIONÁRIO'],
    array_map(fn($s) => [
        $s['titulo'],
    ...
    ['TÍTULO'=>'40%', 'VALOR (R$)'=>'15%', 'FORMA DE PAGAMENTO'=>'20%', 'DATA CAIXA'=>'10%', 'FUNCIONÁRIO'=>'15%']
);

renderTable($pdf, 'Depósitos',
    ['DATA CAIXA', 'DATA CADASTRO', 'VALOR (R$)', 'TIPO', 'FUNCIONÁRIO'],
    array_map(fn($d) => [
    ...
    ['DATA CAIXA'=>'20%', 'DATA CADASTRO'=>'20%', 'VALOR (R$)'=>'15%', 'TIPO'=>'20%', 'FUNCIONÁRIO'=>'25%']
);
...
ob_clean();
$pdf->Output('Fechamento_Caixa_Unificado_'.$data.'.pdf', 'I');
?>
