...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($responseData['resumos'][0]['numeroSelo'])) {
        foreach ($responseData['resumos'] as $selo) {
            ...
            // Save the seal in the selos_requerimentos table
            $stmt = $conn->prepare("INSERT INTO selos_requerimentos (requerimento_id, selo_id) VALUES (?, ?)");
            $stmt->bind_param("ii", $numeroControle, $seloId);
            $stmt->execute();
            $stmt->close();

            // Atualizar os dados na tabela de requerimentos
            $stmt = $conn->prepare("UPDATE requerimentos SET selo = ?, id_selo = ? WHERE id = ?");
            $stmt->bind_param("sii", $numero_selo, $seloId, $numeroControle);
            $stmt->execute();
            $stmt->close();
        }
