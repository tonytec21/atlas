...
<div id="main" class="main-content">
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3 class="mb-0">Cadastro de Imóveis</h3>
            <div>
        ...
        <form id="cadastroImovelForm">
            ...
            <div class="form-row">
                <div class="form-group col-md-3">
                    ...
                    <input type="number" class="form-control" id="areaDoLote" name="area_do_lote" step="0.01" required>
                </div>
                <div class="form-group col-md-3">
                    <label for="perimetro">Perímetro (m)</label>
                    <input type="number" class="form-control" id="perimetro" name="perimetro" step="0.01">
                </div>
                <div class="form-group col-md-3">
                    <label for="areaConstruida">Área Construída (m²)</label>
                    <input type="number" class="form-control" id="areaConstruida" name="areaConstruida" step="0.01">
                </div>
                <div class="form-group col-md-3">
    ...
    <script>
        $(document).ready(function () {
            ...
            function toggleSalvarButton(enable) {
                $('#cadastroImovelForm button[type="submit"]').prop('disabled', !enable);
            }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        try {
                            ...
                            // Se o CPF já está associado a um imóvel
                            if (data.exists) {
                                Swal.fire(
                                    'Atenção!',
                                    `Já existe um imóvel cadastrado para o CPF informado (${cpf}) como proprietário ${data.proprietario}.`,
                                    'warning'
                                );
                                $('#proprietarioNome').val('');
                                $('#conjugeFields').addClass('hidden');
            ...
            $('#cpfConjuge').on('blur', function () {
                ...
                $.ajax({
                    url: 'buscar_proprietario.php', // Reutiliza o mesmo endpoint para buscar pelo CPF
                    method: 'POST',
                    data: { cpf },
                    success: function (response) {
                        try {
                            // Verifica se a resposta já é um objeto ou uma string JSON
                            const data = typeof response === 'string' ? JSON.parse(response) : response;
            ...
            $('#cadastroImovelForm').on('submit', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    url: 'salvar_imovel.php',
                    method: 'POST',
                    data: formData,
                    success: function (response) {
                        if (response.success) {
                            Swal.fire('Sucesso!', 'Imóvel cadastrado com sucesso.', 'success').then(() => location.reload());
                        } else {
                            Swal.fire('Erro!', response.message || 'Erro ao salvar os dados.', 'error');
                        }
                    },
                    error: function () {
