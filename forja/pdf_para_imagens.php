...
try {
    ...
    $r = forja_pdf_para_imagens($ups[0]['path'], $_POST['formato'] ?? 'png', $_POST['dpi'] ?? 150);
    $nome = 'imagens_' . preg_replace('~[^A-Za-z0-9_\-]~', '_', pathinfo($ups[0]['nome'], PATHINFO_FILENAME)) . '.zip';
    $token = forja_registrar_saida($r['zip'], $nome);
    echo json_encode(['status' => 'success', 'token' => $token, 'paginas' => $r['paginas']]);
