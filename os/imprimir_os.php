...
function maskCpfCnpj($valor){
    ...
    // muito curto? mascara todos os dígitos
    ...
    for ($i = 0; $i < strlen($s); $i++){
        ...
        if (ctype_digit($ch)){
            ...
            $result .= $ch; // mantém pontos, barras e hífens
        }
    }
    return $result;
}
...
if (isset($_GET['id'])) {
    ...
    // Cabeçalho fixo SEM a coluna CPF/CNPJ
    function adicionarCabecalhoTabelaContas(&$pdf, $numContas) {
        ...
        $html = '<table border="0.1" cellpadding="4">
            <thead>
                <tr>
                    <th style="width: 12%; text-align: center; font-size: 8.5px;"><b>Banco</b></th>
                    <th style="width: 9%; text-align: center; font-size: 8.5px;"><b>Agência</b></th>
                    <th style="width: 8%; text-align: center; font-size: 8.5px;"><b>Tipo de Conta</b></th>
                    <th style="width: 10%; text-align: center; font-size: 8.5px;"><b>Nº Conta</b></th>
                    <th style="width: 24%; text-align: center; font-size: 8.5px;"><b>Titular da Conta</b></th>
                    <th style="width: 25%; text-align: center; font-size: 8.5px;"><b>Chave PIX</b></th>
                    <th style="width: 11%; text-align: center; font-size: 8.5px;"><b>QR Code PIX</b></th>
                </tr>
            </thead>
            <tbody>';
        return $html;
    ...
    if (!empty($contas)) {
        ...
        foreach ($contas as $index => $conta) {
            // Linha SEM CPF/CNPJ
            $rowHtml = '<tr>
                <td style="width: 12%; font-size: 8.5px;">' . $conta['banco'] . '</td>
                <td style="width: 9%; font-size: 8.5px;">' . $conta['agencia'] . '</td>
                <td style="width: 8%; font-size: 8.5px;">' . $conta['tipo_conta'] . '</td>
                <td style="width: 10%; font-size: 8.5px;">' . $conta['numero_conta'] . '</td>
                <td style="width: 24%; font-size: 8.5px;">' . $conta['titular_conta'] . '</td>
                <td style="width: 25%; font-size: 8.5px;">' . $conta['chave_pix'] . '</td>
                <td style="width: 11%; font-size: 8.5px; text-align: center;"><img src="@' . $conta['qr_code_pix'] . '" height="60" /></td>
            </tr>';
