...
FROM (SELECT ID,dataProtocolo,dataAto,strLivro,strSelo FROM tdpj_td_registro_protocolo WHERE strSelo is not null and strLivro = '00001' and dataAto = '$dataAto') AS a  group BY a.dataAto;
");
...
print_r($strAnotacoes);
    $stmt = $pdo->prepare("INSERT IGNORE into tdpj_td_protocolo_encerramento_diario(dataAto,strAnotacoes,strLivro,dataProtocolo) values ('$dataAto','$strAnotacoes','$strLivro','$dataProtocolo')");
    $stmt->execute();
